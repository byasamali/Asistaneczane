...
const App = () => {
    ...
    if (activeTool) {
        return (
            <div className="animate-fade-in bg-slate-50 min-h-screen">
                <div className="flex items-center p-4 glass border-b sticky top-0 z-50">
                    <button 
                        onClick={() => setActiveTool(null)}
                        className="p-2 -ml-2 text-teal-600 font-bold flex items-center gap-1 active:scale-95 transition-all"
                    >
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col max-w-md mx-auto relative overflow-x-hidden font-sans">
      {/* Header */}
      {!activeTool && (
        <header className="bg-white/90 backdrop-blur-md px-6 py-5 flex items-center justify-between border-b border-slate-100 sticky top-0 z-40">
            <div className="flex items-center gap-2.5">
                <div className="bg-teal-600 p-2 rounded-2xl text-white shadow-lg shadow-teal-100">
                    <HeartPulse size={22} />
                </div>
                <div>
                    <h1 className="font-heading font-extrabold text-lg text-slate-800 leading-none">Eczacı Asistanı</h1>
                    <span className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Klinik & Finans</span>
                </div>
            </div>
      ...
      {/* Bottom Navigation */}
      {!activeTool && (
        <nav className="fixed bottom-0 left-0 right-0 glass border-t border-slate-100 px-8 py-4 flex justify-between items-center z-50 max-w-md mx-auto rounded-t-[2.5rem]">
...
const NavButton = ({ active, onClick, icon, label }: { active: boolean, onClick: () => void, icon: React.ReactNode, label: string }) => (
    <button 
        onClick={onClick}
        className={`flex flex-col items-center gap-1.5 transition-all relative ${active ? 'text-teal-600' : 'text-slate-400'}`}
    >
        <div className={`p-1 rounded-xl transition-all ${active ? 'bg-teal-50 shadow-sm' : ''}`}>
            {icon}
        </div>
        <span className={`text-[9px] font-black uppercase tracking-tighter ${active ? 'opacity-100' : 'opacity-70'}`}>{label}</span>
        {active && <div className="absolute -bottom-1 w-1 h-1 bg-teal-600 rounded-full"></div>}
    </button>
);
    ...
    return (
        <div className="p-6 animate-fade-in space-y-8">
            ...
            <div 
                ...
                className="bg-gradient-to-br from-orange-500 to-rose-500 rounded-4xl p-7 text-white shadow-2xl shadow-orange-100 relative overflow-hidden active:scale-[0.98] transition-all cursor-pointer border border-white/20"
            >
            ...
            <section>
                <div className="flex justify-between items-end mb-5 px-1">
                    <h3 className="text-slate-800 text-lg font-heading font-extrabold tracking-tight">Favori Araçlar</h3>
                    <button onClick={() => setActiveTab(Tab.Calculators)} className="text-teal-600 text-xs font-black uppercase tracking-widest">Tümü</button>
                </div>
...
const CalculatorList = ({ onSelect }: { onSelect: (t: Tool) => void }) => (
    <div className="p-6 animate-fade-in">
        <h2 className="text-2xl font-heading font-black text-slate-800 mb-8 tracking-tight">Klinik Hesaplar</h2>
        
        <div className="mb-10">
            <h3 className="text-[11px] font-black text-slate-300 uppercase tracking-[0.2em] mb-5 ml-1">Klinik Ölçümler</h3>
            <div className="space-y-4">
                <ToolRow icon={<Baby className="text-rose-500" />} title="WHO Z-Skoru" desc="Gelişim Takibi (0-19 Yaş)" onClick={() => onSelect(Tool.WHO)} />
                <ToolRow icon={<Activity className="text-emerald-500" />} title="VKİ Hesapla" desc="Sağlık Durumu Analizi" onClick={() => onSelect(Tool.BMI)} />
                <ToolRow icon={<Calculator className="text-indigo-500" />} title="Vücut Yüzey Alanı" desc="BSA (Mosteller Formülü)" onClick={() => onSelect(Tool.BSA)} />
                <ToolRow icon={<Droplets className="text-blue-500" />} title="Düzeltilmiş Kalsiyum" desc="Albümin Düzeltme Hesabı" onClick={() => onSelect(Tool.Calcium)} />
            </div>
        </div>
...
const ToolCard = ({ icon, title, onClick, color }: { icon: React.ReactNode, title: string, onClick: () => void, color: string }) => (
    <button onClick={onClick} className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-col items-center gap-4 active:scale-95 active:shadow-inner transition-all text-center group">
        <div className={`p-4 ${color} rounded-3xl group-active:scale-110 transition-transform shadow-sm`}>{icon}</div>
        <span className="font-heading font-bold text-slate-800 text-sm tracking-tight">{title}</span>
    </button>
);

const ToolRow = ({ icon, title, desc, onClick }: { icon: React.ReactNode, title: string, desc: string, onClick: () => void }) => (
    <button onClick={onClick} className="w-full bg-white p-5 rounded-3xl shadow-sm border border-slate-100 flex items-center gap-5 active:bg-slate-50 active:scale-[0.98] transition-all text-left group">
        <div className="p-4 bg-slate-50 rounded-2xl group-active:bg-white transition-colors border border-slate-100">{icon}</div>
        <div className="flex-1">
            <div className="font-heading font-bold text-slate-800 tracking-tight leading-tight">{title}</div>
            <div className="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-0.5">{desc}</div>
        </div>
    ...
    return (
        <div className="p-8 text-center animate-fade-in space-y-8">
            <div className="relative inline-block">
                <div className="w-28 h-28 bg-teal-50 rounded-4xl flex items-center justify-center mx-auto shadow-inner border-4 border-white">
            ...
            <div>
                <h2 className="text-3xl font-heading font-black text-slate-800 tracking-tight">Ecz. Burak YAŞAMALI</h2>
                <p className="text-teal-600 font-black mt-1 uppercase text-[10px] tracking-[0.3em]">Software & Pharmacy</p>
            </div>
            
            <div className="bg-white p-7 rounded-4xl shadow-sm border border-slate-100 text-left">
                ...
                <a 
                    ...
                    target="_blank"
                    className="flex items-center justify-center gap-3 bg-teal-600 text-white font-black py-4 rounded-2xl shadow-xl shadow-teal-100 hover:bg-teal-700 active:scale-95 transition-all text-sm uppercase tracking-widest"
                >
            ...
            <div 
                ...
                className="bg-orange-50/50 border border-orange-100 p-7 rounded-4xl text-left relative overflow-hidden shadow-sm cursor-pointer active:scale-[0.99] transition-all"
            >
                <div className="relative z-10">
                    <div className="flex items-center gap-2 mb-3">
                        <Tag size={16} className="text-orange-600" />
                        <h4 className="font-black text-orange-800 uppercase tracking-[0.2em] text-[10px]">TAVSİYE EDİLEN</h4>
                    </div>
                    <h5 className="font-heading font-black text-slate-800 text-xl mb-1 tracking-tight">Pratiket Etiket Sistemi</h5>
                    <p className="text-xs text-slate-500 font-bold leading-relaxed mb-6">
                        Eczaneniz için profesyonel etiketleme ve bilgilendirme çözümü.
                    </p>
                    <div className="flex items-center justify-between gap-4">
                        <div className="flex flex-col">
                           <div className="text-orange-600 font-heading font-black text-2xl tracking-tighter">1.500 ₺ <span className="text-[10px] text-orange-300">/ YIL</span></div>
                        </div>
                        <button className="bg-orange-600 text-white px-5 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-orange-100 active:scale-95 transition-all">
                            İNCELE
                        </button>
                    </div>
                </div>
                <Smartphone className="absolute -bottom-6 -right-6 text-orange-200 opacity-20" size={140} />
            </div>
        </div>
    );
};
